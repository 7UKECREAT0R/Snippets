...
public static class GlobalHotkeys
{
    ...
    private static readonly Dictionary<int, Tuple<object?, Action<object?>>> hotkeys = new();
    private static int lastUsedID;

    /// <summary>
    ///     Register a hotkey.
    /// </summary>
    public static HotkeyContract RegisterHotkey(IntPtr windowHandle, KeyModifiers modifiers, Keys key, object? passIn,
    ...
    public static bool ProcessMessage(ref Message m)
    {
        ...
        if (!hotkeys.TryGetValue(pressedId, out Tuple<object?, Action<object?>>? action))
            return false;
    ...
    ///     current thread rather than with a particular window.
    /// </param>
...
public class HotkeyContract : IDisposable
{
    ...
    public void Dispose()
    {
        if (!this.isDisposed)
        {
            ...
            GlobalHotkeys.ReleaseHotkey(this.hotkeyId, this.windowHandle);
        }
    }
}